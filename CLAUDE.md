...
- `app.py` chegou a **5.273 linhas** (componentes v2 com CSS/JS como strings Python). A modularização é incremental, em `ui/`: **passo 1** extraiu todo o CSS/JS/SVG (33 constantes) para **`ui/estilos.py`** (~1.9k linhas); **passo 2** extraiu 49 helpers de formatação/lógica pura + 7 constantes de dados para **`ui/formato.py`** (~810 linhas). O `app.py` importa os dois de volta e hoje tem **~2.645 linhas**. Regras dessa fatia: os módulos `ui/` **não importam nada do `app.py`** (sem ciclo) e são livres de runtime — só entra função que **não usa `st` nem os globais pós-login** (`df`, `TOTAL`, `modo_conectado`, `USER`). Armadilha vista no passo 2: funções de I/O com caminho ancorado em `__file__` (`_n_fontes_radar`, `_ler_candidatas_avaliadas`, `_atualizar_status_candidata`, `_aprovar_fonte_no_config`) **ficam no `app.py`** — movê-las para `ui/` mudaria a raiz do path e quebraria em silêncio. Não modularizar durante fase de entrega.
...
**Faxina (só depois da entrega):** unificar as paletas de etapa; modularização do `app.py` — passo 1 (CSS/JS → `ui/estilos.py`) e passo 2 (helpers puros → `ui/formato.py`) feitos; próximos passos (telas/modais por painel) só fora de entrega, pois exigem refatorar como o estado de runtime circula.
